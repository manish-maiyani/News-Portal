...
<title>News Portal - View Deleted News</title>
...
<?php
	...
	if($_GET['id'] != '')
	{
	    ...
	    mysqli_query($mysqli, "UPDATE news SET Status='Delete', DT='$s' WHERE id='".$_GET['id']."'");
	}
	include('h-cat_news.php'); 
	$cat = mysqli_query($mysqli,"SELECT news.*, categories.Categories_Type FROM news join categories on news.Cat_ID=categories.id WHERE Status='Delete' ORDER BY news.ID DESC");
?>           
<div class="container">         
	<div id="wrapper1">
        <h1>View Deleted News</h1>   
	</div>    
	...
	<table width="100%" cellpadding="7">
		...
		<?php 
			...
			$cat = mysqli_query($mysqli,"SELECT news.*, categories.Categories_Type FROM news join categories on news.Cat_ID=categories.id WHERE Status='Delete' ORDER BY news.ID DESC limit $page1,6");
			$i=$page2;
		...
		<tr align="center">
            ...
            <td><a href="news_del_delete.php?id=<?php echo $row['ID']; ?>" onclick="return confirm('Are you sure, you want to delete it?')"><font color="red">DELETE</font></a></td>
        </tr>
...
<?php
	$cat = mysqli_query($mysqli,"SELECT news.*, categories.Categories_Type FROM news join categories on news.Cat_ID=categories.id WHERE Status='Delete'");
    ...
    <?php
		...
		{ ?>
			<a href = "viewdelnews.php?page=1"> << </a>
		<?php }
     	...
     	if($page<=3)
     	{
     		...
     		for($b=1 ; $b <= $s ; $b++)
			{?>	
				<a href = "viewdelnews.php?page=<?php echo $b; ?>" <?php if($page == $b) { echo ' class="active"'; } ?> > <?php echo $b." "; ?> </a>
			<?php }
		}
		...
		if(!in_array($page, $lar) && $a>5) 
			{?>
				<a href = "viewdelnews.php?page=<?php echo $a; ?>"> >> </a>
		<?php }
	?>	
</div>
</div>
